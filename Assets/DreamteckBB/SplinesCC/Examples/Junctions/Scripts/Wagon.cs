namespace Dreamteck.Splines.Examples
{
    ...
    public class Wagon : MonoBehaviour
    {
        ...
        public double startPercentFrame;
        public double endPercentFrame;
        public float travelLenghtFrame;
        Wagon front;
        SplineSegment segment, tempSegment;

        private void OnTriggerEnter(Collider other)
        {
            TrainEngine engine = transform.parent.GetChild(0).gameObject.transform.GetComponent<TrainEngine>();
            if (other.gameObject.CompareTag("Tunnel"))
            {
                engine.OnCollisionAction(other.transform.GetChild(0));
            }
        }
        private void Awake()
        {
        ...
        public void UpdateTracer(SplineTracer newTracer)
        {
            tracer = newTracer;
            if (isEngine) SetupRecursively(null, new SplineSegment(tracer.spline, -1, tracer.direction));
        ...
        public void UpdateWagon(Wagon _front, SplineComputer splineComputer, Spline.Direction dir)
        {

            SetupRecursively(_front, new SplineSegment(splineComputer, -1, dir));
            tracer.spline = splineComputer;
            //tracer.TravelWithOffset(1,tracer.spline.CalculateLength(),Spline.Direction.Backward,offset);
            UpdateOffset();
            if (back != null) back.UpdateWagon(this, splineComputer, dir);
        }
        Wagon GetRootWagon()
        {
            Wagon current = this;
        ...
        void ApplyOffset()
        {
            ...
            startPercentFrame = tracer.GetPercent();
            ...
            SplineComputer splineCurrent = segment.spline;
            ...
            if (Mathf.Approximately(totalMoved, offset))
            {
                ...
                endPercentFrame = tracer.GetPercent();
                if (splineCurrent == segment.spline)
                    travelLenghtFrame = startPercentFrame <= endPercentFrame ? tracer.CalculateLength(startPercentFrame, endPercentFrame) : tracer.CalculateLength(endPercentFrame, startPercentFrame);
                else
                    travelLenghtFrame = 0;
                return;
            }
        ...
        void ApplyTracer(SplineComputer spline, double percent, Spline.Direction direction)
        {
            ...
            if ((rebuild && tracer.ClipPercent(percent) == 0) || front.isstop)
            {
                if (front.travelLenghtFrame < 1 && tracer.direction == Spline.Direction.Forward)
                {
                    tracer.SetDistance(tracer.CalculateLength(0, tracer.GetPercent()) + front.travelLenghtFrame);
                }
                else if (front.travelLenghtFrame < 1 && tracer.direction == Spline.Direction.Backward)
                {
                    tracer.SetDistance(tracer.CalculateLength(0, tracer.GetPercent()) - front.travelLenghtFrame);
